...
| SEC-02 | Inbound authorization | API scope → trust → visibility → Agent policy → conversation mode → action approval；own auto / trusted confirm / unknown deny | SEC-01, DISC-02 | ✅ done | ✅ code-review skill |
| SEC-03 | Offline delivery semantics | queued/delivered/accepted/denied/done；ack 后删除；TTL expired；幂等和顺序测试 | PROTO-01 | ✅ done | ✅ code-review skill |
...
| EXT-01 | External Agent contract | 以 `skills/agentlink-platform/SKILL.md` 为契约补齐 discovery/send/stream/error API conformance tests | API-01, API-02 | ✅ done | ✅ code-review skill |
...
| STD-01 | Standard adapters | 将 AgentLink identity/capability 映射到 Google A2A/MCP/ACP | EXT-01 | ✅ done | ✅ code-review skill |
| OPS-01 | Relay metadata hardening | retention、size padding 评估、滥用检测、自托管运维基线 | TEST-01 | ✅ done | ✅ code-review skill |
