...
function loadSkillContent(): string {
  ...
  const candidates = [
    ...
    resolve(process.cwd(), "skills/agentlink-platform/SKILL.md"),
    resolve(process.cwd(), "../skills/agentlink-platform/SKILL.md"),
    resolve(process.cwd(), "../../skills/agentlink-platform/SKILL.md"),
    // from compiled file: packages/server/dist/routes/skill.js -> ../../../../skills/
    resolve(dirname(fileURLToPath(import.meta.url)), "../../../../skills/agentlink-platform/SKILL.md"),
    resolve(dirname(fileURLToPath(import.meta.url)), "../../../skills/agentlink-platform/SKILL.md"),
    // from source: packages/server/src/routes/skill.ts -> ../../../../skills/
    resolve(dirname(fileURLToPath(import.meta.url)), "../../skills/agentlink-platform/SKILL.md"),
    // tauri resources (production)
    resolve((process as typeof process & { resourcesPath?: string }).resourcesPath ?? "", "skills/agentlink-platform/SKILL.md"),
  ];
...
export function registerSkillRoutes(app: FastifyInstance): void {
  ...
  app.get("/api/skill/meta", async () => {
    ...
    return {
      available: true,
      title: titleMatch?.[1] ?? "AgentLink Platform Skill",
