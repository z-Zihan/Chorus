...
export function CatalogModal({ open, onOpenChange }: CatalogModalProps) {
  ...
  const selectedRecipe =
    selectedEntry?.installRecipes.find((recipe) => recipe.method === recipeMethod) ??
        ...
        <div className="flex h-16 shrink-0 items-center gap-3 border-b border-[var(--border-color)] px-5 md:px-8">
          ...
          <div className="min-w-0 flex-1">
            <DialogTitle className="text-lg">
              {selectedEntry?.name ?? t("catalog.title")}
            </DialogTitle>
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="mx-auto grid w-full max-w-5xl gap-8 px-5 py-8 md:grid-cols-[1fr_320px] md:px-8">
              <div>
                <div className="mb-6 flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-[var(--text-tertiary)]">
                      {selectedEntry.publisher.name} · {selectedEntry.license}
                    </p>
                  </div>
                </div>
                ...
                <section className="mb-6">
                  ...
                  <ul className="space-y-2">
                    {selectedEntry.permissions.map((permission) => (
                      ...
                      >
                        <Check className="mt-0.5 h-4 w-4 shrink-0 text-[var(--accent-hover)]" />
                        {permission}
                      </li>
                    ))}
                  </ul>
                ...
                <a
                  className="inline-flex items-center gap-1.5 text-sm text-[var(--accent-hover)] hover:underline"
              ...
              <aside className="rounded-xl border border-[var(--border-color)] bg-[var(--bg-base)] p-5">
                {selectedEntry.installed && !installation ? (
                  <div className="flex items-center gap-2 text-sm text-[var(--text-secondary)]">
                    <Check className="h-5 w-5 text-[var(--status-online)]" />
                    {t("catalog.installed")}
                  </div>
                ) : selectedEntry.detected && !installation ? (
                  <div className="space-y-4">
                    <p className="text-sm text-[var(--text-secondary)]">
                      {t("catalog.detectedReady")}
                    </p>
                    <Button
                      className="w-full"
                      onClick={() =>
                        void adoptDetectedAgent(selectedEntry.descriptorId ?? selectedEntry.id)
                      }
                    >
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-xs leading-5 text-[var(--text-tertiary)]">
                      {t("catalog.confirmDescription")}
                    </p>
                    {selectedEntry.kind === "api-connector" ? (
                      <p className="mt-4 rounded-lg bg-[var(--bg-elevated)] p-3 font-mono text-xs">
                        {t("catalog.noCommand")}
                      </p>
                    ) : (
                      <code className="mt-4 block overflow-x-auto rounded-lg bg-[var(--bg-elevated)] p-3 text-xs text-[var(--text-primary)]">
                        {formatCommand(selectedRecipe)}
                      </code>
                    )}
                    <div className="mt-5 flex gap-2">
                      ...
                      <Button
                        className="flex-1"
                        disabled={selectedEntry.kind === "api-connector" && !apiKey.trim()}
                        onClick={() =>
                          void installAgent(selectedEntry.id, {
                  ...
                  <div className="space-y-4">
                    {selectedEntry.kind === "api-connector" && (
                      <PasswordInput
                        label={t("catalog.apiKey")}
                        value={apiKey}
                        onChange={setApiKey}
                        placeholder={t("catalog.apiKeyPlaceholder")}
                      />
                    )}
                    {selectedEntry.installRecipes.length > 1 && (
                      <label className="block text-sm">
                        <span className="mb-2 block text-[var(--text-secondary)]">
                          {t("catalog.installMethod")}
                        </span>
                        <select
                          value={recipeMethod}
                          onChange={(event) => setRecipeMethod(event.target.value as InstallMethod)}
                          className="h-10 w-full rounded-lg border border-[var(--border-color)] bg-[var(--bg-surface)] px-3 text-sm"
                        >
                          {selectedEntry.installRecipes.map((recipe) => (
                            <option key={recipe.method} value={recipe.method}>
                              {recipe.method}
                            </option>
                          ))}
                        </select>
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {filteredEntries.map((entry) => (
                    <button
                      ...
                      className="rounded-xl border border-[var(--border-color)] bg-[var(--bg-surface)] p-4 text-left transition hover:border-[var(--accent-color)] hover:bg-[var(--bg-hover)]"
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            {!entry.installed && entry.detected && (
                              <span className="text-xs text-[var(--status-busy)]">
                                {t("catalog.detected")}
                              </span>
                            )}
                          </div>
...
function EntryIcon({ entry, large = false }: { entry: CatalogEntry; large?: boolean }) {
  ...
  return (
    <div
      className={cn(
        "flex shrink-0 items-center justify-center rounded-xl bg-[var(--bg-elevated)] text-[var(--accent-hover)]",
...
function KindBadge({ entry }: { entry: CatalogEntry }) {
  const { t } = useTranslation("common");
  return (
    <span className="mt-1 inline-block rounded-full bg-[var(--bg-elevated)] px-2 py-0.5 text-[10px] font-medium uppercase text-[var(--text-tertiary)]">
      {t(`catalog.kinds.${entry.kind}`)}
    </span>
  );
}
  ...
  return (
    <div>
      <div className="flex items-center justify-between text-sm">
        <span>
          {installation.stage === "error"
            ? t("catalog.failed")
            : installation.stage === "done"
              ? t("catalog.installed")
              : t("catalog.installing")}
        </span>
        <span>{t("catalog.progress", { progress: installation.progress })}</span>
      </div>
      ...
      <p className="mt-3 text-xs text-[var(--text-tertiary)]">
        {t(`catalog.stages.${installation.stage}`)}
      </p>
      {installation.error && (
        <p className="mt-2 break-words text-xs text-[var(--status-error)]">{installation.error}</p>
      )}
      {!["done", "error"].includes(installation.stage) && (
        <Button variant="secondary" className="mt-5 w-full" onClick={onCancel}>
          {t("catalog.cancelInstall")}
        </Button>
      )}
    </div>
  );
...
function preferredRecipe(entry: CatalogEntry | null): InstallRecipe | undefined {
  ...
  const method: InstallMethod = platform.includes("mac")
    ? "brew"
...
function formatCommand(recipe?: InstallRecipe): string {
  if (!recipe) return "";
  return [recipe.executable, ...recipe.args]
