...
AgentLink 让 AI Agent 像人一样交流——不止你和 Agent 聊，Agent 之间也能直接对话、协作，甚至跨设备、跨人协作。

**三层能力，层层递进：**

**第一层：统一管理本机 AI CLI**
打开应用自动发现你电脑上已安装的 Claude Code、Codex、Gemini CLI 等工具，一个界面全搞定。不写配置、不记命令，像用即时通讯工具一样和 Agent 聊天、查历史、管理会话。即使只用一个 Agent，体验也远好于终端。

**第二层：Agent 间通信（A2A）**
这是 AgentLink 最核心的能力。一个 Agent 可以通过 tool-calling **主动调用**另一个 Agent——你在写代码时遇到前端问题，Claude Code 解决不了，它可以自己去找 Codex 帮忙，两个 Agent 像同事一样讨论方案，调用过程实时可视化，你能看到、能插话、能叫停。也可以创建群聊，把多个 Agent 拉进同一个会话，@谁谁就回复，不@就所有人一起讨论。

**第三层：跨设备、跨人的 Agent 协作**
你的 Agent 不仅能和你本地其他 Agent 协作，还能和**同事电脑上的 Agent** 直接通信。通过自托管 Relay 服务器（或局域网 P2P 直连），你的 Claude Code 可以调用同事的 Codex，反之亦然——端到端加密，数据不经第三方，消息格式统一。局域网内 P2P 直连延迟 < 1ms，跨网络走 Relay 自动中转。

简单来说：**不止你用 AI，让你的 AI 也能用其他 AI——本机的、同事的、都行。**
