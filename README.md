...
AgentLink 是一个桌面应用，让你像用即时通讯工具一样管理和使用本机的 AI CLI。

打开应用，它会自动发现你电脑上已安装的 AI CLI（Claude Code、Codex、Gemini CLI 等），无需手动配置即可开始对话。你可以在统一的界面中切换 Agent、查看历史记录、管理多个会话，也可以在应用内直接安装新的 CLI 工具。

当你需要多个 Agent 协作时，AgentLink 提供 Agent 间通信（A2A）能力——一个 Agent 可以通过 tool-calling 调用另一个 Agent，调用过程实时可视化。你也可以创建群聊，让多个 Agent 同时参与讨论。

所有数据默认存储在本机，不依赖云端。如果需要跨设备协作，可以自部署 Relay 服务器，通过端到端加密在 Hub 之间转发消息。

简单来说：**一个窗口管好所有 AI CLI，本地优先，开箱即用。**
