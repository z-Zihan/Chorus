...
- 调用链限制深度、并发和时间，并支持级联取消。单次 Agent→Agent 调用默认 5 分钟且可配置 1–30 分钟；整条自动协作任务默认最多 20 分钟，并保证不短于已配置的单次调用时间。
- 每条用户任务的 Agent→Agent 自动交接默认最多 12 轮，用户可在设置页配置 1–50；达到上限必须明确提示并停止下一次调用。
- 每次交接必须携带原始目标、相关上下文、具体子任务、已有证据和期望交付物；不得以寒暄、致谢或重复结论触发下一轮。
